...
class VendorInfoView extends React.Component {
  render() {
    ...
    const currentSType = _.get(
      usageDataProvider,
      'harvestingConfig.sushiConfig.serviceType',
      ''
    );
    const serviceType = harvesterImpls
      ? harvesterImpls.find((e) => e.value === currentSType)
      : [];
    ...
    return (
      <Row>
        <Col xs={3}>
          <KeyValue
            label={
              <FormattedMessage id="ui-erm-usage.information.harvestVia" />
            }
            value="Sushi"
          />
        </Col>
        ...
        <Col xs={3}>
          <KeyValue
            ...
            value={
              usageDataProvider.harvestingConfig?.sushiConfig?.serviceUrl ?? (
                <NoValue />
              )
            }
          />
