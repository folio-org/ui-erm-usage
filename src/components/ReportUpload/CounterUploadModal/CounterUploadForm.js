...
class CounterUpload extends React.Component {
  ...
  selectFile = (acceptedFiles) => {
    ...
    this.getBase64(currentFile, (result) => {
      // this.props.form.mutators.setContent(result);
      this.props.form.change('contents.data', result);
    });
  ...
  changeReportEditedManually = () => {
    this.setState((prevState) => ({
      reportEditedManually: !prevState.reportEditedManually,
    }));
  };
  ...
  render() {
    ...
    if (
      (this.state.reportEditedManually === true &&
        this.state.editReason !== '') ||
      (this.state.reportEditedManually === false &&
        this.state.editReason === '')
    ) {
...
export default injectIntl(
  stripesConnect(
    stripesFinalForm({
      ...
      mutators: {
        setContent: (args, state, tools) => {
          tools.changeValue(state, 'contents.data', () => args[1]);
        },
      },
    })(CounterUpload)
  )
